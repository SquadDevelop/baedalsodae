...
public class OrderQueryCondition {
                ...
                .or(
                        order.id.in(
                                JPAExpressions.select(orderItem.order.id)
                                        .from(orderItem)
                                        .where(
                                                orderItem.nameSnapshot.containsIgnoreCase(
                                                        keyword))));
    }

    public static BooleanExpression orderNoEqIgnoreCase(OrderListQuery query) {
        return query.orderNo() != null ? order.orderNo.equalsIgnoreCase(query.orderNo()) : null;
    }
    ...
    public static BooleanExpression cursorCondition(OrderListQuery query) {
        ...
        return order.createdAt
                ...
                .or(order.createdAt.eq(query.cursorCreatedAt()).and(order.id.lt(query.cursorId())));
    }
}
