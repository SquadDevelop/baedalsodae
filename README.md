...
| 팀원 | 주제 | 요약 |
|---|---|---|
| **신혜원** | 커서 기반 정렬 보장 | 복합 정렬 기준에서 동일 값 발생 시 `id` 타이브레이킹 + `crossBoundary` OR 조건으로 페이지 경계 정확성 보장 |
| **이현빈** | refreshToken + Redis 블랙리스트 | 무상태성 유지를 위해 accessToken/refreshToken 분리, 로그아웃된 토큰을 Redis TTL로 블랙리스트 관리 |
| **정재빈** | 아웃박스 패턴 + 이벤트 폴링 | 트랜잭션 안정성을 위해 `@TransactionalEventListener` 대신 아웃박스 패턴 + 스케줄러 폴링 선택 |
| **하지혜** | 비관적 락 도입 | 메뉴 순서 변경 시 다중 row 동시 수정 → 낙관적 락 재시도 복잡도 문제로 비관적 락 선택 |
| **한병두** | pgvector + STT 기반 메뉴 추천 | 음성 입력(STT) → 텍스트 임베딩 → pgvector 유사도 검색으로 자연어 기반 메뉴 추천 구현 |
